...
const Journal = () => {
  ...
  useEffect(() => {
    const loadEntries = async () => {
      try {
        const savedEntries = await offlineDataService.getJournalEntries();
        if (savedEntries.length > 0) {
          setEntries(savedEntries);
        } else {
          // If no entries in IndexedDB, use mock data
  ...
  const handleSaveEntry = async () => {
    ...
    try {
      ...
      // Save to IndexedDB
      const entryId = await offlineDataService.saveJournalEntry(newEntry);
      
      // Update entry with ID returned from IndexedDB
      const savedEntry = { 
        ...newEntry, 
        id: entryId
      };
      ...
      if (isOffline) {
        console.log('Entry saved offline. Will sync when connection is restored.');
      }
    } catch (error) {
