...
io.on('connection', (socket) => {
  ...
  socket.on('join-circle', (circleId: string) => {
    ...
    logger.info(`User ${socket.id} joined circle ${circleId}`);
  });
  ...
  socket.on('chat-message', (message: { circleId: string; text: string }) => {
    ...
    logger.debug(`Message in circle ${message.circleId}: ${message.text}`);
  });
  ...
  socket.on('ritual-update', (data: { circleId: string; action: string }) => {
    ...
    logger.info(`Ritual update in circle ${data.circleId}: ${data.action}`);
  });
...
server.listen(PORT, () => {
  ...
  logAppEvent('SERVER_STARTED', { port: PORT, env: process.env.NODE_ENV });
});
